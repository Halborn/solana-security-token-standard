...
pub enum SecurityTokenInstruction {
    ...
    /// 3. `[signer]` Original mint creator account that matches the mint authority PDA seeds
    /// 4. `[writable]` SPL Token mint account
    /// 5. `[writable]` Mint authority PDA account (owned by this program, must be the close authority)
    /// 6. `[writable]` Token account that will be closed
    /// 7. `[]` SPL Token 2022 program account
    Close = 12,
}
...
impl TryFrom<u8> for SecurityTokenInstruction {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            12 => Ok(SecurityTokenInstruction::Close),
